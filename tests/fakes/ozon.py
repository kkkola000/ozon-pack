"""Подделка Ozon Seller API — только для тестов.

Детерминированные отправления, товары, штрихкоды и возвраты: панель работает
исключительно на данных площадки, и без предсказуемого источника проверить
сборку было бы нечем. В самой панели этого файла нет.
"""
