...
# Значения из примеров в документации. Домены example.* зарезервированы, и
# Let's Encrypt отказывает по ним и в сертификате, и в регистрации почты — но
# узнаётся это лишь на середине выпуска, после установки nginx и правки
# конфигов, сообщением от certbot, в котором не видно, что виноват пример.
case "$DOMAIN" in
  *example.com|*example.org|*example.net|ВАШ-ДОМЕН|*ПРИМЕР*)
    die "Домен «$DOMAIN» — из примера в документации, Let's Encrypt его не выдаст. Укажите свой: --domain panel.мой-магазин.ru" ;;
esac
case "$EMAIL" in
  ""|*@example.com|*@example.org|*@example.net|ПОЧТА|*ПРИМЕР*)
    [ -z "$EMAIL" ] || die "Почта «$EMAIL» — из примера в документации, Let's Encrypt её не примет. Укажите свою: --email вы@ваш-домен.ru" ;;
esac
