...
3. **Два адреса за одним разделом.** `/returns` и `/avito/returns` показывают
   одно и то же разными шаблонами; лист печати тоже продублирован.

## Этапы перехода

Каждый заканчивается зелёным прогоном (591 тест) и отдельным коммитом. После
любого можно остановиться — код рабочий.

1. **Переезд по пакетам.** `markets/<код>/` — клиент, сборка, роуты как есть.
   `core/` — общие модули как есть. Логика не меняется, только пути и импорты.
2. **Расшивка общих файлов.** `store`, `sync`, `labels`, DDL, миграции,
   `options`, озоновская часть `return_acts` и `routes/returns` разъезжаются по
   площадкам.
3. **Реестр.** `Market`, `registry.py`, объявления трёх площадок. Убираются
   двадцать развилок. Здесь исчезают три ошибки выше.
4. **Общие страницы.** Один раздел возвратов, одно рабочее место
   (`market_pack.html` + `market_pack.js`), шаблоны и статика площадок в их
   пакетах.
5. **Тесты по папкам** зеркально коду.
