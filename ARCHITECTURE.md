...
│   │   ├── product_links.py        — сопоставление карточек разных кабинетов в один товар склада
...
│   │   │                             «Обновить каталоги» — по кабинетам, которые объявили источник
...
## Сборка идёт по фильтру, а не по кабинету

Кабинет в шапке на «Сборке» ничего не решает — иначе этикетка Avito в кабинете
Ozon давала бы «такого отправления нет», хотя заказ есть. Границы сборки задаёт
фильтр кабинетов (`?shop=<номер>`): «Все заказы» — по всем кабинетам, выбран
кабинет — в его границах. По кабинетам, а не по площадкам: у одной площадки их
бывает несколько, и склад собирает их раздельно.

Куда уйдёт скан, решает `app/core/packing.py`, и площадок по именам он не знает:

```python
# Workspace каждой площадки объявляет четыре вещи про сборку
owner=pack.owner,          # («label»|«product»|«known», номер) — чей это код; ничего не меняет
scan=pack.scan,            # один скан в этом кабинете
release=pack.release,      # отмена начатой сборки
complete=pack.complete_active,   # «завершить без скана»; None — площадка так не умеет
label=...,                 # наклейка одного заказа на печать: (PDF, имя файла)
```

Правила, в порядке применения:

1. начатая сборка сильнее фильтра — коробка уже в руках;
2. наклейка чужого кабинета посреди сборки — стоп с указанием магазина
   (товар при этом не в счёт: он продаётся в нескольких магазинах);
3. сборки нет — код ищется среди кабинетов под фильтром, а при нескольких
   попаданиях берётся заказ, который стоит первым в общем списке; открыть
   нечего нигде — отвечает кабинет, где товар лежит в заказе («known»).

`owner` площадка строит теми же функциями, что и `scan`: ответ обязан совпасть
с тем, что скан потом сделает. Расхождение и было ошибкой 1.40.0 — кабинет с
заказом в «Ожидает сборки» молчал, и скан уходил в кабинет из шапки. Кабинет
в шапке в выборе не участвует вовсе.
...
1. **«Товары» → 409 в кабинетах Avito и Яндекса.** Ссылка в шапке показывается
   ...
   (`routes/products.py`, все маршруты). ✅ Раздел стал общим на все кабинеты:
   каталог показывается целиком, с фильтром по кабинетам, и кабинета своей
   площадки не требует — склад на складе один.
