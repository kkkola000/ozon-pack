...
class Workspace:
    ...
    # Чей это код. owner(account_id, code) -> («label» | «product», номер
    # заказа) либо None, если код этому кабинету не принадлежит. Спрашивается
    # до скана и ничего не меняет: по ответам ядро выбирает кабинет, а при
    # нескольких «product» — первый по списку заказов.
    #
    # «label» и «product» различаются не для красоты: наклейка чужого кабинета
    # посреди сборки — это стоп, а товар, который продаётся и в соседнем
    # магазине, — обычная позиция открытого заказа.
    owner: Callable[[int, str], tuple[str, str] | None] = lambda _account_id, _code: None
