...
на площадку. «К сборке» здесь — это «Ожидает отгрузки» в «Заказах»: на сборку
попадают только такие заказы, у всех площадок. «Ожидает сборки» в списке есть,
но сканом не открывается — его сначала подтверждают на площадке. Собранный —
«Собран» в обоих местах. Площадок по именам тут нет.
"""
...
def row(account_id: int, number, *, goods: str | None, quantity, deadline: str | None,
    ...
    in_work — «к сборке»: заказ можно собирать прямо сейчас («Ожидает
    отгрузки»). Остальное из списка не исчезает, но по умолчанию скрыто
    галочкой: сборщику незачем смотреть на то, что сканом всё равно не откроется.
    """
    return {
